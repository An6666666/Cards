...
private void HideIdleOverlays()
{
    ...
    private Player cachedPlayer;                 // 快取玩家參考
    private Vector2Int lastKnownPlayerPos;       // 上一次玩家位置
    private bool hasLastKnownPlayerPos = false;  // 是否初始化過
...
public virtual void DecideNextIntent(Player player)
{
    ...
    protected void UpdateIntentIcon()
    {
        if (intentIconRenderer == null)
            return;
        ...
        switch (nextIntent.type)
        {
            ...
            case EnemyIntentType.Skill:
                icon = intentSkillSprite != null ? intentSkillSprite : intentAttackSprite;
                break;
            default:
