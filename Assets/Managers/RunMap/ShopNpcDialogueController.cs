...
public class ShopNpcDialogueController : MonoBehaviour // 掛在場景物件上的對話控制腳本
{
    ...
    [Header("Custom Shop Dialogue")] // 可在 Inspector 分開設定不同商店互動的台詞池
    [SerializeField] private string[] removeCardPurchaseLines =
    {
        "這張{card}我幫你處理掉了，牌組會更俐落。",
        "收你{gold}金，{card}已經從你的牌組裡消失了。",
        "不要的牌早點割捨是好事，{card}我替你清掉了。",
        "這種累贅牌少一張，手氣都會順一點。"
    };
    ...
    public void NotifyPurchase(string itemName, int spentGold) // 外部通知：購買成功（卡牌/遺物）
    {
    ...
    public void NotifyCardRemovalPurchase(string removedCardName, int spentGold)
    {
        int safeSpend = Mathf.Max(0, spentGold);
        int totalSpend = LoadShopNpcStat(ShopNpcTotalSpendKey);

        if (safeSpend > 0)
        {
            totalSpend += safeSpend;
            SaveShopNpcStat(ShopNpcTotalSpendKey, totalSpend);
        }

        ShowNpcLine(BuildCardRemovalNpcLine(removedCardName, safeSpend, totalSpend));
    }
    ...
    {
        string safeItemName = SanitizeInlineItemName(itemName); // 安全商品名稱（避免空字串與內嵌換行）

        if (totalSpend >= 150) // 累積消費很高
            return NormalizeInlineDialogue(PickRandomLine( // 從高消費台詞池隨機抽一句
        ...
        if (spentGold >= 12) // 本次消費偏高
            return NormalizeInlineDialogue(PickRandomLine( // 從高價台詞池隨機抽一句
                $"真有眼光，{safeItemName}值這{spentGold}金幣。",
                $"這件{safeItemName}很搶手呢。",
                $"好選擇，{safeItemName}是我們店裡的精品。"));

        if (spentGold >= 8) // 本次消費中等
            return NormalizeInlineDialogue(PickRandomLine( // 從中價台詞池隨機抽一句
                $"成交，{safeItemName}已為你準備好。",
                $"好，{safeItemName}我這就交給你。",
                $"{safeItemName}已經是你的了，收好。"));

        return NormalizeInlineDialogue(PickRandomLine( // 其餘視為一般消費
            $"好交易，這是你的{safeItemName}，拿好了。",
            $"成交，{safeItemName}給你了。",
            $"謝謝光顧，{safeItemName}歸你。"));
    }

    private string BuildCardRemovalNpcLine(string cardName, int spentGold, int totalSpend)
    {
        string safeCardName = SanitizeInlineItemName(cardName);
        string configuredLine = BuildConfiguredRemovalLine(safeCardName, spentGold, totalSpend);
        if (!string.IsNullOrWhiteSpace(configuredLine))
            return configuredLine;

        if (totalSpend >= 150)
        {
            return NormalizeInlineDialogue(PickRandomLine(
                $"像你這樣肯花的客人，{safeCardName}這種拖油瓶當然得幫你清乾淨。",
                $"收下這筆錢，{safeCardName}我替你處理得乾乾淨淨。",
                $"{safeCardName}已經不會再卡你的節奏了，這筆花得值。"));
        }

        if (spentGold >= 12)
        {
            return NormalizeInlineDialogue(PickRandomLine(
                $"收你{spentGold}金，{safeCardName}這張我幫你消掉。",
                $"{safeCardName}已經從牌組裡除名了，之後抽牌會更舒服。",
                $"刀子我磨好了，{safeCardName}這張就幫你切掉。"));
        }

        return NormalizeInlineDialogue(PickRandomLine(
            $"{safeCardName}我替你清掉了，牌組輕一點，出手也會俐落些。",
            $"好，{safeCardName}不留了，之後就看你牌運更順。",
            $"這張{safeCardName}已經處理掉，接下來就別再惦記它了。"));
    }

    private static string SanitizeInlineItemName(string itemName)
    {
        const string fallbackName = "這件商品";

        if (string.IsNullOrWhiteSpace(itemName))
            return fallbackName;

        System.Text.StringBuilder builder = new System.Text.StringBuilder(itemName.Length);
        bool previousWasWhitespace = false;

        for (int i = 0; i < itemName.Length; i++)
        {
            char currentChar = itemName[i];
        ...
        if (!ContainsAsciiLetter(sanitized))
    ...
    private string BuildConfiguredRemovalLine(string cardName, int spentGold, int totalSpend)
    {
        if (removeCardPurchaseLines == null || removeCardPurchaseLines.Length == 0)
            return string.Empty;

        string template = PickRandomLine(removeCardPurchaseLines);
        if (string.IsNullOrWhiteSpace(template))
            return string.Empty;

        return NormalizeInlineDialogue(template
            .Replace("{card}", cardName)
            .Replace("{item}", cardName)
            .Replace("{gold}", spentGold.ToString())
            .Replace("{total}", totalSpend.ToString())
            .Trim());
    }

    private string NormalizeInlineDialogue(string text)
    {
    ...
    public void ShowScriptedLine(string line)
    {
        ShowNpcLine(line);
    }

    public RectTransform ShopNpcRectTransform
    {
        get
        {
            ResolveFallbackReferences();
            return shopNpcImage != null ? shopNpcImage.transform as RectTransform : null;
        }
    }

    public RectTransform MessageRootRectTransform
    {
        get
        {
            ResolveFallbackReferences();
            GameObject root = GetMessageRoot();
            return root != null ? root.transform as RectTransform : null;
        }
    }

    public void ShowPinnedLine(string line)
    {
        ...
        EnsureMessageCanvasGroup();
        if (messageVisibilityTween != null)
        {
            messageVisibilityTween.Kill(false);
            messageVisibilityTween = null;
        }
        ...
        messageText.text = line.Trim();
    }

    public void ClearPinnedLine()
    {
        if (messageVisibilityTween != null)
        {
            messageVisibilityTween.Kill(false);
